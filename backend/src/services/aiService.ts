...
// Set hardcoded API keys for now (for demo purposes only)
const HUGGING_FACE_API_KEY = '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';
const ELEVEN_LABS_API_KEY = 'sk_94564a491cce488d779604a3ae45477631d3deec81f84421'; 
const GOOGLE_GEMINI_API_KEY = 'AIzaSyB6HODSktcm4xCrC2puRb2KbexDA3jZmWQ';

// Initialize services with API keys
if (GOOGLE_GEMINI_API_KEY || process.env.GOOGLE_GEMINI_API_KEY) {
  genAI = new GoogleGenerativeAI(GOOGLE_GEMINI_API_KEY || process.env.GOOGLE_GEMINI_API_KEY || '');
  logger.info('Google Gemini API initialized');
}

if (HUGGING_FACE_API_KEY || process.env.HUGGINGFACE_API_KEY) {
  hf = new HfInference(HUGGING_FACE_API_KEY || process.env.HUGGINGFACE_API_KEY || '');
  logger.info('Hugging Face API initialized');
}

if (ELEVEN_LABS_API_KEY || process.env.ELEVENLABS_API_KEY) {
  elevenlabs = new ElevenLabsClient({
    apiKey: ELEVEN_LABS_API_KEY || process.env.ELEVENLABS_API_KEY || '',
  });
